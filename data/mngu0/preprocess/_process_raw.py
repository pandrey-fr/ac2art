...
"""Set of functions to transform raw mngu0 data."""
...
def adjust_filesets():
    ...
    As the provided filesets lists are based on a transformed version of
...
DOC_TRANSFORM_DETAILS = """
    The transformations include the following:
...
def transform_all_utterances(
    ...
    audio_forms, articulators_list = scan_transformation_parameters(
...
transform_all_utterances.__doc__ = transform_all_utterances.__doc__.format(
    DOC_TRANSFORM_DETAILS, DOC_TRANSFORM_ARGUMENTS
)


def transform_utterance_data(
        utterance, n_coeff=40, audio_forms=None, articulators_list=None,
        ema_sampling_rate=200, audio_frames_size=200
    ):
    """Transform acoustic and articulatory data of a given mngu0 utterance.
    {0}
    ...
    # Conduct the actual data transformation.
    _transform_utterance_data(
        utterance, n_coeff, audio_forms, articulators_list,
        ema_sampling_rate, audio_frames_size
    )
...
transform_utterance_data.__doc__ = transform_utterance_data.__doc__.format(
    DOC_TRANSFORM_DETAILS, DOC_TRANSFORM_ARGUMENTS
)


def _transform_utterance_data(
        utterance, n_coeff, audio_forms, articulators_list,
        ema_sampling_rate, audio_frames_size
    ):
    """Transform acoustic and articulatory data of a given mngu0 utterance.

    This function serves as a dependency for `transform_all_utterances`
    and `transform_utterance_data`, avoiding to check again and again
...
def scan_transformation_parameters(
